...
describe('templates', () => {
  describe('registry', () => {
    ...
    it('301st category contains the iso2 template and no asn template', () => {
      const t301st = getTemplatesByCategory('301st');
      const ids = t301st.map((t) => t.id);
      expect(ids).toContain('301st.iso2.csv');
      expect(ids).not.toContain('301st.asn.csv');
    });

    it('every template declares country inputType', () => {
      for (const tpl of ALL_TEMPLATES) {
        expect(tpl.inputType).toBe('country');
      }
    });

    it('getTemplatesByCategoryAndInput filters by both', () => {
      const generic = getTemplatesByCategoryAndInput('generic', 'country');
      expect(generic.length).toBe(4);
      expect(generic.every((t) => t.inputType === 'country' && t.category === 'generic')).toBe(true);
    });
