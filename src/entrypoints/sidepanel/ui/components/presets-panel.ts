...
export function createPresetsPanel(container: HTMLElement, store: Store): { destroy(): void } {
  let draftName = '';
  let renamePresetId: string | null = null;
  let renameValue = '';
  ...
  function render(): void {
    ...
    saveWrap.className = 'presets-panel__save';
    ...
    saveWrap.append(saveInput, saveBtn);
    ...
    container.append(header, saveWrap);
    ...
    for (const preset of sorted) {
      ...
      renameBtn.textContent = renamePresetId === preset.id ? 'Cancel' : 'Rename';
